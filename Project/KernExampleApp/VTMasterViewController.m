...
- (void)viewDidLoad
{
    ...
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemAdd target:self action:@selector(dropDatabaseAndLoadTestStore)];
}

- (void)dropDatabaseAndLoadTestStore {
    [Kern drop];
    [Kern saveContext];
    
    [Kern setupSqliteStackWithName:@"TestStore" hasJournaling:TRUE];
}
...
- (void)configureCell:(UITableViewCell *)cell atIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.textLabel.text = [NSString stringWithFormat:@"%@ %@ (%@)", user.firstName, user.lastName, user.luckyNumber];
